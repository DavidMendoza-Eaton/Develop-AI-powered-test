      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantItems = details.participants.length
          ? details.participants
              .map(
                (email) =>
                  `<li><span class="participant-email">${escapeHtml(email)}</span><button class="delete-btn" data-activity="${escapeHtml(name)}" data-email="${escapeHtml(email)}" title="Remove participant">&#x2715;</button></li>`
              )
              .join("")
